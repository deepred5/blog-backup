...
前置的基础知识的确有些~~看晕人~~，不过在平时的开发过程中，我们基本无感知这些内容。

简单一句话概况：移动端适配就是在进行**<font color="#6495ed">屏幕宽度</font>的等比例缩放**：
...
```
注意的是：我们取 `100px` 作为设计稿的1rem，是因为方便计算，比如设计稿`250px`，我们就可以很容易的计算出为`2.5rem`。

我们当然也可以把 `50px` 作为设计稿的1rem，这时设计稿的`250px`，就要写成`5rem`。
